...
process Align_samples {
  ...
  #/usr/local/miniconda/bin/bwa mem -t !{task.cpus} -M -R \'@RG\\tID:group1\\tSM:!{R1}\\tPL:illumina\\tLB:lib1\\tPU:unit1\' -p -L [17,17] lava_ref.fasta !{R1} > !{R1}.sam
	# Sorts SAM.
  ...
  #/usr/local/miniconda/bin/samtools mpileup --max-depth 500000 -f lava_ref.fasta !{R1}.bam > !{R1}.pileup
...
process Pipeline_prep {
  ...
  #/usr/local/miniconda/bin/bwa mem -t ${task.cpus}  -M -R \'@RG\\tID:group1\\tSM:${CONTROL_FASTQ}\\tPL:illumina\\tLB:lib1\\tPU:unit1\' -p -L [17,17] lava_ref.fasta ${CONTROL_FASTQ} > ${CONTROL_FASTQ}.sam
  ...
  #/usr/local/miniconda/bin/samtools mpileup --max-depth 500000 -f lava_ref.fasta ${CONTROL_FASTQ}.bam > ${CONTROL_FASTQ}.pileup
	"""
}
...
process Create_VCF {
  ...
  #  then
  #    awk -F":" '($12+0)>=1{print}' !{R1}.avinput > !{R1}_2.avinput
  #  else
  #    awk -v af=!{ALLELE_FREQ} -F":" '($12+0)>=!{ALLELE_FREQ}{print}' !{R1}.avinput > !{R1}_2.avinput
  #fi

  #mv !{R1}_2.avinput !{R1}.avinput

  python3 ANNOVAR_Replacement.py !{R1}.avinput lava_ref.gff consensus.fasta
...
process Ref_done {
  ...
  #cp !{FIRST_FILE}.exonic_variant_function > !{FIRST_FILE}.txt
...
process Generate_output {
    ...
    #python3 ${GENOME_PROTEIN_PLOTS} final_Visual.csv proteins.csv reads.csv . "Plot"
		mkdir vcf_files
		mv *.vcf vcf_files
		mkdir genomecov
    ...
    #python3 ${GENOME_PROTEIN_PLOTS} final_Visual.csv proteins.csv reads.csv . "Plot" -categorical
		mkdir vcf_files
		mv *.vcf vcf_files
		mkdir genomecov
		mv *.genomecov genomecov
