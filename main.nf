...
//VISUAL = file("$workflow.projectDir/bin/final_Visual.csv")
CONSENSUS = file("$workflow.projectDir/bin/consensus2.fasta")
...
def nfcoreHeader() {
    ...
    return """
      ...
      `7MMF'            db `7MMF'   `7MF' db
        ...
        MM            ,V^MM.  VM:   ,V  ,V^MM.
        MM           ,M  `MM   MM.  M' ,M  `MM
        MM      ,    AbmmmqMA  `MM A'  AbmmmqMA
        MM     ,M   A'     VML  :MM;  A'     VML
      .JMMmmmmMMM .AMA.   .AMMA. VF .AMA.   .AMMA.
